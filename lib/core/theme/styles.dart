...
double getScaleFactor(context) {
  ...
  double minWidth = 550; // Minimum width for scaling
  double maxWidth = 1920; // Maximum width for scaling

  // Calculate a normalized value between 0 and 1 based on width
  double normalizedWidth = (width - minWidth) / (maxWidth - minWidth);

  // Use lerp to interpolate between scale factors for different ranges
  double scaleFactorA = width / 550; // Scale factor for small screens
  double scaleFactorB =
      width / 1000; // Scale factor for medium screens (around 700)
  double scaleFactorC = width / 1920; // Scale factor for large screens

  // Interpolate between B and C for smoother transition
  double lerpedScaleFactor = lerp(scaleFactorB, scaleFactorC, normalizedWidth);
