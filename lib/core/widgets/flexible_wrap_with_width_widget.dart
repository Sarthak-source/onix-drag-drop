...
class FlexibleWrapWidgetWithWidth extends StatelessWidget {
  const FlexibleWrapWidgetWithWidth({
    super.key,
    required this.children,
    required this.itemWidths, // List of widths for each child
    this.textDirection,
  ...
  /// The list of widths for each item in the wrap.
    ...
    return LayoutBuilder(
      builder: (context, constraint) {
        ...
        if (constraint.maxWidth.isFinite) {
          final totalItemWidth = itemWidths.isNotEmpty ? itemWidths.reduce((a, b) => a + b) : 0.0;
          final averageItemWidth = totalItemWidth / itemWidths.length;
          final widthWithSpacing = averageItemWidth + spacing;

          final items = (constraint.maxWidth / widthWithSpacing).floor();
          final remainder = constraint.maxWidth.remainder(widthWithSpacing);
        ...
        return Wrap(
          ...
          children: List.generate(children.length, (index) {
            double width = itemWidths.length > index ? itemWidths[index] : 0.0;
            return SizedBox(
              width: noWrapIndex.contains(index) ? null : width + extraWidth,
              child: Padding(
